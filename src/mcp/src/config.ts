...
export interface LokkaConfig {
  tenants: TenantConfig[];
}

export function loadConfig(configPath: string): LokkaConfig {
  const content = readFileSync(configPath, "utf-8");
  const config = JSON.parse(content) as LokkaConfig;
...
export function selectTenant(config: LokkaConfig, tenantName?: string): TenantConfig {
  if (tenantName) {
    const found = config.tenants.find(
      (t) => t.name.toLowerCase() === tenantName.toLowerCase()
    );
    if (!found) {
      const available = config.tenants.map((t) => t.name).join(", ");
      throw new Error(
        `Tenant '${tenantName}' not found in config. Available tenants: ${available}`
      );
    }
    return found;
  }
...
export function tenantConfigToAuthConfig(tenant: TenantConfig): AuthConfig {
  let mode: AuthMode;
  ...
  if (rawAuthMode !== undefined && rawAuthMode !== null) {
    switch (rawAuthMode.toLowerCase()) {
      ...
      default:
        throw new Error(
          `Invalid authMode '${rawAuthMode}' for tenant '${tenant.name}'. ` +
